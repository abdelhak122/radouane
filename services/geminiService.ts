...
export const analyzeProduct = async (imageFile: File, language: string, productCategory: string, apiKey: string): Promise<AnalysisResult> => {
  ...
  const textPrompt = language === 'ar' 
    ? `الرجاء تحليل صورة مكونات المنتج هذه. فئة المنتج هي: ${productCategory}`
    : `Please analyze this product ingredients image. The product category is: ${productCategory}`;
