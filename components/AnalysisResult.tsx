...
const AnalysisResultDisplay: React.FC<AnalysisResultProps> = ({ result }) => {
  ...
  const { productName, productCategory, overallScore, verdict, summary, negatives = [], positives = [], questionable = [], analysisConfidence } = result;
        ...
        {productCategory && (
            <div className="mt-4">
                <span className="inline-block bg-gray-200 dark:bg-gray-700 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {t('results.productCategory')}: {productCategory}
                </span>
            </div>
        )}
      </div>
